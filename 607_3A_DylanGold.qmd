...
estimated_movies <- as.data.frame(estimated_review) %>% 
  rowwise() %>% 
  mutate(Predicted_Movie = colnames(estimated_movies)[which.max(c_across(movies))])
